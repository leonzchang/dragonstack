...
export default class Breeder {
  static breederDragon({ matron, patron }: { matron: Dragon; patron: Dragon }) {
    const matronTraits = matron.traits;
    const patronTraits = patron.traits;

    const babyTraits: { traitType: string; traitValue: string }[] = [];

    matronTraits.forEach(({ traitType, traitValue }) => {
      const matronTrait = traitValue;

      const patronTrait = patronTraits.find((trait) => trait.traitType === traitType)!.traitValue; //non-null
  ...
  // Else, pick patron.
  static pickTrait({ matronTrait, patronTrait }: { matronTrait: string; patronTrait: string }) {
