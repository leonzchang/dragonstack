...
const accountReducer = (state:reduxAccountSate = DEFAULT_ACCOUNT ,action:accountAction) =>{
    switch(action.type){
        ...
        case ACCOUNT.FETCH_SUCCESS:
            return {...state, status:fetchState.success, message:action.message, loggedIn:true}
        case ACCOUNT.FETCH_LOGOUT_SUCCESS:
            return {...state, status:fetchState.success, message:action.message, loggedIn:false}
        case ACCOUNT.FETCH_AUTHENTICATED_SUCCESS:
            return {...state, status:fetchState.success, message:action.message, loggedIn:action.authenticated}
        default:
            return state 
    }
}
